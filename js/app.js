...
function _getAudioCtx() {
  if (!audioCtx) {
    try {
      ...
      /* browser blocks audio — silently skip */
    }
  }
  return audioCtx;
}
...
function toggleDarkMode() {
  ...
  DOM.darkToggle().title = state.darkMode
    ...
    try {
      if (file.type === "application/pdf" || /\.(pdf)$/i.test(file.name)) {
        ...
        for (let i = 0; i < pages.length; i++) {
          const pageName =
            pages.length > 1 ? `${file.name} - Page ${i + 1}` : file.name;
...
async function loadPdfPages(file) {
  if (typeof pdfjsLib === "undefined")
    throw new Error("PDF support library is not available.");
  ...
  return new Promise((resolve, reject) => {
    ...
    if (file.type === "image/tiff" || /\.(tiff?)$/i.test(file.name)) {
      if (typeof UTIF === "undefined") {
        return reject(
          new Error("TIFF support library is not available / blocked."),
        );
      }
...
function drawScanToCanvas(img) {
  ...
  if (w > CONFIG.MAX_CANVAS_DIM || h > CONFIG.MAX_CANVAS_DIM) {
    const ratio = Math.min(
      CONFIG.MAX_CANVAS_DIM / w,
      CONFIG.MAX_CANVAS_DIM / h,
    );
...
function rotateScan(degrees) {
  if (state.activeFileIdx < 0) {
    showToast("Upload a scan first.", "warning");
    return;
  }
...
function initZoomWheel() {
  DOM.canvasScroll().addEventListener(
    "wheel",
    (e) => {
...
function sampleColorAt(cx, cy) {
  ...
  const data = ctx.getImageData(
    clamp(pos.x, 0, canvas.width - 1),
    clamp(pos.y, 0, canvas.height - 1),
    1,
    1,
  ).data;
...
function spawnWorker() {
  ...
  try {
    ...
    console.warn(
      "Web Worker spawn failed — will fallback to main thread:",
      err.message,
    );
    state.worker = null;
  }
...
async function runDetection() {
  ...
  if (state.worker) {
    ...
    state.worker.postMessage(
      { type: "DETECT_PHOTOS", imageData: detectImageData, options: opts },
...
function applyDetectedBoxes(boxes) {
  if (!boxes || boxes.length === 0) {
    showToast(
      "No photos detected. Try adjusting sensitivity or draw manually.",
      "warning",
    );
    return;
  }
  ...
  if (typeof gtag !== "undefined")
    gtag("event", "photos_detected", { count: state.boxes.length });
  showToast(
    `Detected ${state.boxes.length} photo${state.boxes.length !== 1 ? "s" : ""}.`,
    "success",
  );
}
...
async function runStraightenAll() {
  ...
  const promises = state.boxes.map(
    (box) =>
      new Promise((resolve) => {
    ...
    if (isSelected) {
      getHandlePoints(box).forEach(([hx, hy, cursor, dir]) => {
        ...
        c.addEventListener("mousedown", (e) => {
          e.stopPropagation();
          startBoxDrag(e, box.id, "handle", dir);
        });
        c.addEventListener(
          "touchstart",
          (e) => {
            e.stopPropagation();
            startBoxTouch(e, box.id, "handle", dir);
          },
          { passive: false },
        );
        g.appendChild(c);
...
function selectBox(id) {
  if (
    typeof CollageManager !== "undefined" &&
    $("collage-modal") &&
    $("collage-modal").getAttribute("aria-hidden") === "false"
  ) {
...
function applyDrag(pt) {
  ...
  if (_drag.type === "move") {
    ...
    if (dir.includes("w")) {
      x = o.x + dx;
      w = Math.max(CONFIG.MIN_BOX_PX, o.w - dx);
    }
    if (dir.includes("n")) {
      y = o.y + dy;
      h = Math.max(CONFIG.MIN_BOX_PX, o.h - dy);
    }
...
function onBoxTouchEnd() {
  _drag = null;
  document.removeEventListener("touchmove", onBoxTouchMove);
  document.removeEventListener("touchend", onBoxTouchEnd);
  renderStrip();
}
...
function initKeyboard() {
  document.addEventListener("keydown", (e) => {
    ...
    switch (e.key) {
      ...
      case "E":
        if (state.selectedBoxId !== null) {
          rotateBox(state.selectedBoxId, 90);
          e.preventDefault();
        }
        break;
      case "q":
...
function finishCropDraw(endX, endY) {
  ...
  const norm = normalizeBox(
    _cropDraw.startX,
    _cropDraw.startY,
    endX - _cropDraw.startX,
    endY - _cropDraw.startY,
  );
...
async function renderStrip() {
  ...
  const thumbPromises = state.boxes.map((box, idx) =>
    createThumbElement(box, idx),
  );
...
async function createThumbElement(box, idx) {
  ...
  const thumbCanvas = await renderBoxToCanvas(box, {
    maxH: CONFIG.THUMBNAIL_H,
  });
  ...
  ov.querySelector('[data-action="rotatecw"]').addEventListener(
    "click",
    (e) => {
      e.stopPropagation();
      rotateBox(box.id, 90);
  ...
  ov.querySelector('[data-action="rotateccw"]').addEventListener(
    "click",
    (e) => {
      e.stopPropagation();
      rotateBox(box.id, -90);
  ...
  ov.querySelector('[data-action="edit"]').addEventListener("click", (e) => {
    e.stopPropagation();
    openMetadataModal(box.id);
  });
  ov.querySelector('[data-action="ocr"]').addEventListener("click", (e) => {
    e.stopPropagation();
    OCRManager.extractText(box.id);
  });
  ov.querySelector('[data-action="preview"]').addEventListener("click", (e) => {
    e.stopPropagation();
    previewBox(box.id);
  });
  ov.querySelector('[data-action="copy"]').addEventListener("click", (e) => {
    e.stopPropagation();
    copyBoxToClipboard(box.id);
  });
  ov.querySelector('[data-action="share"]').addEventListener("click", (e) => {
    e.stopPropagation();
    SocialManager.openModal(box.id);
  });
  ov.querySelector('[data-action="delete"]').addEventListener("click", (e) => {
    e.stopPropagation();
    deleteBox(box.id);
  });
...
async function renderBoxToCanvas(box, opts = {}) {
  ...
  const doEnhance =
    enhance !== undefined ? enhance : DOM.autoEnhanceToggle()?.checked;
  const filmType =
    opts.filmType !== undefined
      ? opts.filmType
      : DOM.filmTypeSelect()?.value || "none";
...
function applySharpen(imgData, amount) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    data[i] = Math.max(
      0,
      Math.min(255, data[i] + mix * (data[i] - blurred[i])),
    );
    data[i + 1] = Math.max(
      0,
      Math.min(255, data[i + 1] + mix * (data[i + 1] - blurred[i + 1])),
    );
    data[i + 2] = Math.max(
      0,
      Math.min(255, data[i + 2] + mix * (data[i + 2] - blurred[i + 2])),
    );
  }
}
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (l > 220) {
        ...
        // Shadows / vegetation
        cr = l * 0.9;
        cg = Math.min(255, l * 1.1);
        cb = l * 0.9;
      } else {
        // Dark shadows (cool)
        cr = l * 0.9;
        cg = l * 0.9;
        cb = Math.min(255, l * 1.1);
      }
...
function convertNegativeCanvas(canvas, filmType) {
  ...
  switch (filmType) {
    case "bw":
      _convertBWNegative(canvas);
      break;
    case "color":
      _convertColorNegative(canvas, filmBase);
      break;
    case "slide":
      _convertSlide(canvas);
      break;
  }
}
...
async function convertNegativeCanvasAsync(canvas, filmType) {
  ...
  return new Promise((resolve, reject) => {
    ...
    state.worker.addEventListener("message", handler);
    state.worker.postMessage({ type: "PROCESS_NEGATIVE", imageData, options }, [
...
async function previewBox(id) {
  ...
  // Before image (unprocessed crop from original)
  const beforeCanvas = await renderBoxToCanvas(box, {
    enhance: false,
    filmType: "none",
    skipTransform: true,
  });
  const beforeUrl = beforeCanvas.toDataURL("image/jpeg", 0.5);

  const idx = state.boxes.indexOf(box);
  DOM.modalTitle().textContent = box.label || `Photo ${idx + 1}`;
  DOM.modalPreviewImg().src = dataUrl;
  $("modal-preview-img-before").src = beforeUrl;
  ...
  $("modal-rotate-cw").addEventListener("click", () => {
    rotateBox(id, 90);
    previewBox(id);
  });
  $("modal-rotate-ccw").addEventListener("click", () => {
    rotateBox(id, -90);
    previewBox(id);
  });
  $("modal-save-one").addEventListener("click", () => {
    ExportManager.saveOne(id);
  });
  $("modal-share-one").addEventListener("click", () => {
    closeModal();
    SocialManager.openModal(id);
  });
  ...
  if (hasEffect) {
    ...
    baBtn.addEventListener("click", async () => {
      ...
      const original = await renderBoxToCanvas(box, {
        enhance: false,
        filmType: "none",
      });
...
const ExportManager = {
  ...
  async injectExifYear(blob, year) {
    try {
      ...
      const exifMarker = [
        ...
        0x00,
        0x00,
        0x00, // offset = 26 from TIFF start (8+2+12+4)
        // Next IFD offset = 0
        0x00,
        0x00,
        0x00,
  ...
  _exportSidecars(box, baseWithoutExt, isZip, zipObj) {
    if (box.audioNote) {
      if (isZip) {
        ...
        setTimeout(
          () =>
            this.triggerDownload(box.audioNote, `${baseWithoutExt}_audio.webm`),
          200,
        );
      }
    }
    if (
      ...
      if (isZip) {
        ...
        setTimeout(
          () => this.triggerDownload(textBlob, `${baseWithoutExt}_meta.txt`),
          400,
        );
      }
    }
      ...
      fetch(box.backImage)
        ...
        .then((blob) => {
          if (isZip) {
            ...
            setTimeout(
              () => this.triggerDownload(blob, `${baseWithoutExt}_back.jpg`),
              600,
            );
          }
        });
  ...
  async saveAll() {
    if (state.boxes.length === 0) {
      showToast(
        "No splits to save. Run detection or draw crop regions first.",
        "warning",
      );
      return;
    }
    ...
    for (const [idx, box] of state.boxes.entries()) {
      ...
      saved++;
      setProgress(
        Math.round((saved / state.boxes.length) * 100),
        `Saving ${saved} of ${state.boxes.length}…`,
      );
    ...
    hideSpinner();
    if (typeof gtag !== "undefined")
      gtag("event", "images_downloaded", { count: saved, type: "individual" });
    showToast(
...
const ZipManager = {
  async saveAllAsZip(boxes, prefix = "photo_", customFormat = null) {
    ...
    for (const [idx, box] of boxes.entries()) {
      ...
      const blob = await ExportManager.canvasToBlob(
        canvas,
        usedFormat,
        quality,
      );
      ...
      setProgress(
        Math.round((done / boxes.length) * 80),
        `Compressing ${done} of ${boxes.length}…`,
      );
    }
    ...
    hideSpinner();
    if (typeof gtag !== "undefined")
      gtag("event", "zip_downloaded", { count: boxes.length });
    showToast(
      `✅ ZIP ready — ${boxes.length} photo${boxes.length !== 1 ? "s" : ""} archived!`,
      "success",
      4000,
    );
  },
...
const SocialManager = {
  ...
  openModal(boxId, presetKey = null) {
    const box = boxId
      ...
      : state.selectedBoxId
        ...
        : state.boxes.length
          ...
          : null;
    if (!box) {
      showToast("No photo detected yet.", "warning");
      return;
    }
    this._activeBoxId = box.id;
    this._activePlatform = presetKey
      ? PLATFORM_PRESETS.find((p) => p.key === presetKey) || PLATFORM_PRESETS[0]
      : PLATFORM_PRESETS[0];
    ...
    $("social-bg-color").addEventListener("input", () =>
      this._refreshPreview(),
    );
    $("social-watermark-text").addEventListener("input", () =>
      this._refreshPreview(),
    );
    $("social-watermark-pos").addEventListener("change", () =>
      this._refreshPreview(),
    );
  ...
  _renderPlatformGrid() {
    ...
    PLATFORM_PRESETS.forEach((preset) => {
      ...
      btn.className =
        "platform-btn" +
        (preset.key === this._activePlatform.key ? " active" : "");
      const dims =
        preset.w && preset.h ? `${preset.w}×${preset.h}` : "Original";
      btn.innerHTML = `
  ...
  async _buildCanvas() {
    ...
    const srcCanvas = await renderBoxToCanvas(box, {
      enhance: DOM.autoEnhanceToggle()?.checked,
    });
  ...
  async nativeShare() {
    if (!navigator.share) {
      showToast(
        "Web Share API is not supported on this browser or connection.",
        "error",
      );
      return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (soundBtn) {
    soundBtn.textContent = soundsMuted ? "🔇" : "🔊";
    soundBtn.onclick = () => {
      soundsMuted = !soundsMuted;
      localStorage.setItem("pss_sounds_muted", soundsMuted);
      soundBtn.textContent = soundsMuted ? "🔇" : "🔊";
      if (!soundsMuted && audioCtx.state === "suspended") audioCtx.resume();
    };
  }
  ...
  if (scrubber) {
    scrubber.oninput = (e) => {
      ...
      const currentVal = state.undoStack.length;
      if (targetVal < currentVal) {
        for (let i = 0; i < currentVal - targetVal; i++)
          BoundingBoxEditor.undo();
      } else if (targetVal > currentVal) {
        for (let i = 0; i < targetVal - currentVal; i++)
          BoundingBoxEditor.redo();
      }
    };
  }

  // Curtain Reveal init
  const curtain = $("curtain-handle");
  if (curtain) {
    const updateCurtain = (x) => {
      const rect = $("curtain-container").getBoundingClientRect();
      const percent = Math.max(
        0,
        Math.min(100, ((x - rect.left) / rect.width) * 100),
      );
      curtain.style.left = `${percent}%`;
      $("curtain-after").style.clipPath = `inset(0 0 0 ${percent}%)`;
    };
    curtain.onmousedown = () => {
      const move = (e) => updateCurtain(e.clientX);
      const stop = () => {
    ...
    // Touch support
    curtain.ontouchmove = (e) => updateCurtain(e.touches[0].clientX);
  }
});
...
const CarouselSplitter = {
  openModal() {
    ...
    document.querySelectorAll(".grid-preset-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        document
          .querySelectorAll(".grid-preset-btn")
          .forEach((b) => b.classList.remove("active"));
        btn.classList.add("active");
  ...
  _getCols() {
    return Math.max(
      1,
      Math.min(10, parseInt($("carousel-cols").value, 10) || 3),
    );
  },
  _getRows() {
    return Math.max(
      1,
      Math.min(10, parseInt($("carousel-rows").value, 10) || 1),
    );
  },
  ...
  _splitTiles(cols, rows) {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        tile
          .getContext("2d")
          .drawImage(srcCanvas, c * tw, r * th, tw, th, 0, 0, tw, th);
  ...
  async splitAndZip() {
    ...
    ExportManager.triggerDownload(
      zipBlob,
      `${baseName}carousel_${cols}x${rows}.zip`,
    );
    hideSpinner();
    showToast(
      `✅ Carousel ZIP ready — ${tiles.length} tiles!`,
      "success",
      4000,
    );
    this.closeModal();
...
function initNegativeTools() {
  ...
  pickBtn.addEventListener("click", () => {
    ...
    if (state.pickingFilmBase) {
      showToast(
        "Click on the orange/unexposed border of the film to calibrate.",
        "info",
        4000,
      );
  ...
  window.openMetadataModal = function (boxId) {
    ...
    requestAnimationFrame(() =>
      $("metadata-modal").setAttribute("aria-hidden", "false"),
    );
  };
  ...
  const OCRManager = {
    ...
    async init() {
      ...
      return new Promise((resolve, reject) => {
        ...
        script.onerror = () => {
          hideSpinner();
          showToast("Failed to load OCR.", "error");
          reject();
        };
    ...
    async extractText(boxId) {
      ...
      try {
        ...
        if (text.trim()) {
          box.meta = box.meta || {};
          box.meta.notes =
            (box.meta.notes ? box.meta.notes + "\n\n" : "") + text.trim();
          showToast("Text extracted and added to Notes!", "success");
          if (
            $("metadata-modal") &&
            $("metadata-modal").getAttribute("aria-hidden") === "false"
          ) {
            $("meta-notes").value = box.meta.notes;
          } else {
  ...
  $("btn-record-audio")?.addEventListener("click", () =>
    AudioNoteManager.startRecording(),
  );
  $("btn-stop-audio")?.addEventListener("click", () =>
    AudioNoteManager.stopRecording(),
  );
  $("btn-delete-audio")?.addEventListener("click", () =>
    AudioNoteManager.deleteNote(),
  );
  ...
  DOM.mainCanvas().addEventListener("click", (e) => {
    ...
    showToast(
      `Calibrated! Base Color: R:${color.r} G:${color.g} B:${color.b}`,
      "success",
    );
...
function initButtons() {
  ...
  DOM.btnAddCrop().addEventListener("click", () =>
    state.cropDrawing ? stopCropMode() : startCropMode(),
  );
  ...
  if (btnZip)
    btnZip.addEventListener("click", () =>
      ZipManager.saveAllAsZip(state.boxes),
    );
  ...
  if (btnCarousel)
    btnCarousel.addEventListener("click", () => CarouselSplitter.openModal());

  // Collage Maker Button
  const btnCollage = $("btn-collage-maker");
  if (btnCollage)
    btnCollage.addEventListener("click", () => CollageManager.openModal());

  // Slideshow Button
  const btnSlideshow = $("btn-slideshow");
  if (btnSlideshow)
    btnSlideshow.addEventListener("click", () => SlideshowManager.start());
  ...
  if (btnDetectAll) {
    btnDetectAll.addEventListener("click", async () => {
      if (state.files.length === 0) {
        showToast("Load some scans first.", "warning");
        return;
      }
      const total = state.files.length;
      showToast(
        `⚡ Running detection on ${total} scan${total !== 1 ? "s" : ""}…`,
        "info",
        3000,
      );
      for (let i = 0; i < total; i++) {
        ...
        setProgress(
          ...
          `Processed ${i + 1} of ${total}…`,
        );
      }
      showToast(
        `✅ Detected photos in all ${total} scan${total !== 1 ? "s" : ""}!`,
        "success",
        4000,
      );
    });
...
function init() {
  ...
  console.log(
    "%cPhotoSplit Studio loaded — zero network requests for your images.",
...
async function saveAsPdf() {
  if (!window.jspdf) {
    showToast("PDF library not loaded yet, try again.", "warning");
    return;
  }
  if (state.boxes.length === 0) {
  ...
  try {
    ...
    for (let i = 0; i < state.boxes.length; i++) {
      const box = state.boxes[i];
      setProgress(
        Math.round((i / state.boxes.length) * 90),
        `Rendering photo ${i + 1} of ${state.boxes.length}…`,
      );
    ...
    showToast(
      `✅ PDF saved — ${state.boxes.length} photo${state.boxes.length !== 1 ? "s" : ""}`,
      "success",
      3000,
    );
  } catch (err) {
